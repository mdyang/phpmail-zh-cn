...
<body>
<form action="upload-img.php?tid=<?php echo $_GET["tid"];?>" method="post" enctype="multipart/form-data" name="form1">
...
if (!empty($_FILES)){
	...
	$pic = $dstpath."/images/".$fname;
	$upTemp = move_uploaded_file($upfile['tmp_name'],$pic);
	if ($upTemp){
		echo "<script language='javascript'>parent.complete_image_upload('success', '".$fname."');</script>";
	}else{
		echo "<script language='javascript'>parent.complete_image_upload('failed', '');</script>";
	}
//	}
}
?>
